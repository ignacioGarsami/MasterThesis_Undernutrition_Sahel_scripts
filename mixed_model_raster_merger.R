...
biomass =  st_read('Biomass_data/BIO_ADM2') 
...
biomass_data = subset(biomass, select=c('admin0Name','admin1Name','admin2Name','AREA','BIO_1998','BIO_1999','BIO_2000','BIO_2001','BIO_2002',
                                        'BIO_2003','BIO_2004','BIO_2005','BIO_2006','BIO_2007','BIO_2008','BIO_2009','BIO_2010','BIO_2011','BIO_2012','BIO_2013',
                                        'BIO_2014','BIO_2015','BIO_2016','BIO_2017','BIO_2018','BIO_2019','geometry'))
...
biomass_data$admin2Name = factor(biomass_data$admin2Name)

#First we add all the admin 2 into admin1
biomass_data = biomass_data %>%
  group_by(admin0Name, admin1Name) %>%
  summarise(AREA = sum(AREA),BIO_1998 = sum(BIO_1998),BIO_1999 = sum(BIO_1999),BIO_2000 = sum(BIO_2000),BIO_2001 = sum(BIO_2001),BIO_2002 = sum(BIO_2002),
            BIO_2003 = sum(BIO_2003),BIO_2004 = sum(BIO_2004),BIO_2005 = sum(BIO_2005),BIO_2005 = sum(BIO_2005),BIO_2006 = sum(BIO_2006),
            BIO_2007 = sum(BIO_2007),BIO_2008 = sum(BIO_2008),BIO_2009 = sum(BIO_2009),BIO_2010 = sum(BIO_2010),BIO_2011 = sum(BIO_2011),
            BIO_2012 = sum(BIO_2012),BIO_2013 = sum(BIO_2013),BIO_2014 = sum(BIO_2014),BIO_2015 = sum(BIO_2015),BIO_2016 = sum(BIO_2016),
            BIO_2017 = sum(BIO_2017),BIO_2018 = sum(BIO_2018),BIO_2019 = sum(BIO_2019), geometry = st_union(geometry))
...
biomass_data = melt(data = biomass_data, id.vars = c('admin0Name','admin1Name', 'Mean','Area','geometry'), measure.vars = c('1998','1999','2000','2001',
                                                                                                                 '2002','2003','2004',
...
zone_1 = subset(chad, admin1Name %in% c("N'Djamena"))
zone_1 = zone_1 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 1', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_2 = subset(chad, admin1Name %in% c('Borkou', 'Ennedi-Est', 'Ennedi Ouest', 'Tibesti', 'Kanem', 'Barh-El-Gazel', 'Lac'))
zone_2 = zone_2 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 2', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_3 = subset(chad, admin1Name %in% c('Guera', 'Batha', 'Salamat'))
zone_3 = zone_3 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 3', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_4 = subset(chad, admin1Name %in%  c('Ouaddai', 'Sila', 'Wadi Fira'))
zone_4 = zone_4 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 4', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_5 = subset(chad, admin1Name %in%  c('Chari-Baguirmi', 'Hadjer-Lamis'))
zone_5 = zone_5 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 5', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_6 = subset(chad, admin1Name %in%  c('Mayo-Kebbi Ouest', 'Mayo Kebbi Est'))
zone_6 = zone_6 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 6', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_7 = subset(chad, admin1Name %in%  c('Logone Occidental', 'Logone Oriental', 'Tandjile'))
zone_7 = zone_7 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 7', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))

zone_8 = subset(chad, admin1Name %in%  c('Moyen-Chari', 'Mandoul'))
zone_8 = zone_8 %>%
  group_by(Year) %>%
  summarise(admin0Name = 'Chad', admin1Name = 'Zone 8', Area = sum(Area),Mean = mean(Mean), Biomass = sum(Biomass),  geometry = st_union(geometry))
...
data = data %>% 
  ...
  select(admin0Name, admin1Name,Mean,Area,Biomass,Year,Mortality, Wasted,Stunted, geometry)
...
#write.csv2(data,'Mixed_model_data/base_data_raster.csv', sep = ';')
data = st_as_sf(data, sf_column_name = 'geometry')
data = st_collection_extract(
  data,
  type = c("POLYGON"),
  warn = FALSE
)

st_write(data, 'Mixed_model_data/base_data_raster', driver="ESRI Shapefile",  layer_options = "GEOMETRY=AS_MULTIPOLYGON") 
