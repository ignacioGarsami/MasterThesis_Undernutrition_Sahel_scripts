---
title: "Final Modeling Notebook WestAfrica"
...
plot(density(na.omit(data$Area)), xlab = 'Area', main = 'Density of Area')
plot(density(na.omit(data$Biomass)), xlab = 'Biomass', main = 'Density of Biomass')
plot(density(na.omit(data$Bmss_nm)), xlab = 'Biomass anomaly', main = 'Density of Biomass anomaly')
...
plot(density(na.omit(data$Wasted)), xlab = 'Wasting prevalence', main = 'Density of wasting prevalence')
plot(density(na.omit(data$Stunted)), xlab = 'Stunting prevalence', main = 'Density of stunting prevalence')
...
ggplot(data) + aes( y = (Bmss_nm), col = admn0Nm) + geom_boxplot() + theme_classic()
...
grid_arrange_shared_legend(plot_mort,plot_wasted,plot_stunted, ncol=2, top = textGrob("Mortality, stunting and wasting prevalences per country",gp=gpar(fontsize=20)))
```

```{r}

ggplot(data = data_wasted) + aes(x = Year, y = Wasted, col = admn0Nm) + geom_smooth(size = 1, alpha = 0.2) + geom_point(alpha = 0.5) + ylab('Wasting prevalence') + 
...
#Corrplot
library(corrplot)
cor1 = cor(na.omit(as.numeric(data[,4:9])))
...
model.lineal.2 = lm(log(Wasted+1) ~  Year + Area + Bmss_nm, data = data_wasted)
...
ggplot(data = world) +
    ...
    coord_sf(xlim =c(30, -20), ylim = c(0, 28), expand = FALSE) + 
    xlab('Longitude') + ylab('Latitude') + 
    ggtitle('Mean residuals of estimated acute malnutrition prevalence in West Africa', subtitle = 'From 1998 to 2018, by region') + 
    labs(fill = 'Estimated residuals') + 
    theme( panel.background = element_rect(fill = 'aliceblue'))
...
plot.title=title(main="Inferred prevalence of acute malnutrition, West Africa",
xlab="longitude",ylab="latitude"), yrange = c(0,27), xrange = c(-20,30))
}
...
#Random intercept with fixed mean
model.mixed.1 =lmer(sqrt(Stunted) ~ Year + Area + Bmss_nm + (1|admn0Nm) + (1|admn1Nm), data = data_stunted, REML = FALSE)
#Intercept varying among group 1 and group 2 within group 1
model.mixed.2 = lmer(sqrt(Stunted) ~ Year + Area + Bmss_nm + (1|admn0Nm/admn0Nm), data = data_stunted, REML = FALSE)
summary(model.mixed.1)
summary(model.mixed.1)
...
```{r}
#Uncorrelated slope and intercept for the year and intercept varying among g1 and g2 withing g1.
model.mixed.3 = lmer(sqrt(Stunted) ~ Year + Area + Bmss_nm + (Year  | admn0Nm) + (Year  | admn1Nm), data = data_stunted, REML = FALSE, na.action = na.omit)
summary(model.mixed.3)
anova(model.mixed.1,model.mixed.3)
...
plot(model.mixed.1 , sqrt(Stunted) ~ fitted(.) , abline = c(0,1))
plot(model.mixed.1 , sqrt(Stunted) ~ fitted(.) | admn0Nm, abline = c(0,1))
plot(model.mixed.1 , sqrt(Stunted) ~ fitted(.) | admn1Nm, abline = c(0,1))
...
model.spam = fitme(sqrt(Stunted) ~ Year + Area + Bmss_nm + (1 | admn0Nm) + (1 | admn1Nm) ,data=data_stunted, method = 'ML')
...
model.spam.2 = fitme(sqrt(Stunted) ~ Year + Area + Bmss_nm + (1 | admn0Nm) + (1 | admn1Nm) +  Matern( 1 | lat + lon), distMatrix = MLdistMat, data=data_stunted.2, method = 'ML')
summary(model.spam.2)
```
...
ggplot(data = world) +
    ...
    coord_sf(xlim =c(30, -20), ylim = c(0, 28), expand = FALSE) + 
    xlab('Longitude') + ylab('Latitude') + 
    ggtitle('Mean residuals of estimated chronic malnutrition prevalence in West Africa', subtitle = 'From 1998 to 2018, by region') + 
    labs(fill = 'Estimated residuals') + 
    theme( panel.background = element_rect(fill = 'aliceblue'))
...
plot(model.spam.2$eta, sqrt(data_stunted$Stunted))
abline(a = 0,b = 1)
...
filled.mapMM(model.spam.2,add.map=TRUE,plot.axes=quote({axis(1);axis(2)}),
decorations=quote(points(pred[,coordinates],pch=15,cex=0.3)),
plot.title=title(main="Inferred prevalence of chronic malnutrition, West Africa",
xlab="longitude",ylab="latitude"), yrange = c(0,27), xrange = c(-20,30))
}
...
filled.mapMM(model.spam.2, nlevels=10, add.map=TRUE, plot.axes=quote({axis(1);axis(2)}),
             variance="respVar",
             plot.title=title(main="Inferred migration propensity of blackcaps",
                               xlab="longitude",ylab="latitude"))

spamm.1 =  fitme(Stunted ~ Year + Area + Bmss_nm + (1 | admn0Nm) + (1 | admn1Nm) +  Matern( 1 | lat + lon), distMatrix = MLdistMat, data=data_stunted.2, method = 'ML')
spamm.2 =  fitme(Stunted ~ Year + Area + Bmss_nm + Area:Bmss_nm + Bmss_nm:Year + (1 | admn0Nm) + (1 | admn1Nm) +  Matern( 1 | lat + lon), distMatrix = MLdistMat, data=data_stunted.2, method = 'ML')

confint(spamm.1, 'Bmss_nm')
